  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12">
            <h1 class="m-0">Tambah Data Posyandu </h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
    ...
    <div class="content">
      <div class="container-fluid">
        ...
        {
                    $smpnamapsd        = trim(mysqli_real_escape_string($con, $_POST['nmposyandu']));
                    $smpndsn       = trim(mysqli_real_escape_string($con, $_POST['dusunpsd']));
                    $smpnrmh       = trim(mysqli_real_escape_string($con, $_POST['tbhrmh']));
                    $smpnrw       = trim(mysqli_real_escape_string($con, $_POST['tbhrw']));
                    $smpnrt       = trim(mysqli_real_escape_string($con, $_POST['tbhrt']));

                    
            $cekdata = mysqli_query($con, "SELECT * FROM tbl_lokasi_posyandu WHERE nama_posyandu='$smpnamapsd'")
            ...
            if (mysqli_num_rows($cekdata) > 0)
            {
              ...
              <script> 
              swal("Error", "Nama Posyandu sudah ada, silahkan ganti Nama yang lain", "error");
            ...
            {
              $ambilidyangbarumasuk = mysqli_query($con,"SELECT id FROM tbl_dusun WHERE nama_dusun='$smpndsn'") or die (mysqli_error($con));
              ...
              $ambilidyangbarumasuk = mysqli_query($con,"SELECT id FROM tbl_rt WHERE no_rt='$smpnrt'") or die (mysqli_error($con));
              $arr = mysqli_fetch_assoc($ambilidyangbarumasuk);              
              if (mysqli_num_rows($ambilidyangbarumasuk)>0)
                  {
                    $erre2  = mysqli_fetch_assoc($ambilidyangbarumasuk);            
              ...
              $ambilidyangbarumasuk = mysqli_query($con,"SELECT id FROM tbl_rt WHERE no_rw='$smpnrw'") or die (mysqli_error($con));
              $arr = mysqli_fetch_assoc($ambilidyangbarumasuk);              
              if (mysqli_num_rows($ambilidyangbarumasuk)>0)
                  {
                    $erre2  = mysqli_fetch_assoc($ambilidyangbarumasuk);            
                ...
                mysqli_query($con, "INSERT INTO tbl_lokasi_posyandu VALUES ('','$smpnamapsd','$kodedusun','$smpndsn','$smpnrmh', '$koderw','$smpnrw', '$kodert','$smpnrt')")
            or die(mysqli_error($con));
            ?>
              <script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>
              <script> 
